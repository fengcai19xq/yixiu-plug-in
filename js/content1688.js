    ...
    if (message == "startAction"){
        if (!isStop){
            ...
            if (accountId == 'undefined' || accountId == ''){
                showErrorDom("获取账号id失败");
                return  ;
            }else{
                var keyToken = "accountId"+accountId;
                chrome.storage.sync.get([keyToken], function (data){
                    console.log('key is '+[keyToken],'\n value is '+data);
                    var resValue = data["accountId"+accountId];
...
function showErrorDom(msg){
    var errorDiv = $('#errorMsgId');
    if(msg==true){
    ...
    if (errorDiv.length>0){
        ...
        var html = "<p id='errorMsgId' style='color: red;font-size: x-large'>"+msg+"</p>";
        para.innerHTML = html ;
        $('.app-container').find('div.header').children('h1')[0].appendChild(para);
    }
...
function createDom(){
    ...
    var html = "<p id='execText' style='color: blue;font-size: x-large'>正在执行中，请勿关闭页面！！！</p>" +
        "<p id='tipTextP' style='color: cadetblue;font-size: large;display: none'>" +
        "<span id='tipText'></span></p>";
    para.innerHTML = html ;
